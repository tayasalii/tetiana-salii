...
export const Tabs = ({ tabs, benefits }) => {
  return (
    <div>
      <Tab.Group>
        <Tab.List className="grid grid-rows-2 grid-cols-2 xl:grid-cols-4 xl:grid-rows-1 gap-x-4 gap-y-3 md:gap-x-5 md:gap-y-8 xl:gap-x-[22px] mb-[25px] md:mb-8 xl:mb-10 w-full mdOnly:w-[464px] mx-auto">
          {tabs.map(({ tabTitle }, id) => (
            <Tab
              key={id}
              className={({ selected }) =>
                classNames(
                  'font-ui_montserrat h-[64px] w-full mdOnly:max-w-[222px] xl:w-[294px] xl:h-[74px] px-[22px] py-[13px] md:py-3 xl:px-[48px] border-[1px] border-ui_purpleLight text-ui_m_body1 md:text-ui_t_body1 xl:text-ui_t_m_body2',
                  selected
